...
public:
  ...
  void onInit() override {
    ...
    if (use_gps) {
      ...
      gnss_sub = nh.subscribe("/fix", 0.2, &HdlLocalizationNodelet::navsat_callback, this);
      heading_sub = nh.subscribe("/heading", 0.2, &HdlLocalizationNodelet::heading_callback, this);
    }
...
private:
  ...
  void initialize_params() {
    ...
    if (utm_file.is_open() && private_nh.param<bool>("use_utm", true) && (private_nh.param<bool>("use_gps", true))) {
      ...
      double longitude;
      double altitude;
      geographic_msgs::GeoPointStampedPtr gps_zero_msg(new geographic_msgs::GeoPointStamped());
      utm_file >> latitude >> longitude >> altitude >> zero_yaw;
      // ROS_INFO("test for latitude:[%f]",latitude);
      ROS_INFO_STREAM(
        "Global map offset by GPS reference coordinates (latitude = " << latitude << ", longitude = " << longitude << ") and  (altitude = " << altitude
                                                                      << ") and (yaw =" << zero_yaw << ")");
      gps_zero_msg->position.latitude = latitude;
      gps_zero_msg->position.longitude = longitude;
      gps_zero_msg->position.altitude = altitude;
      geodesy::fromMsg(gps_zero_msg->position, utm);
      Eigen::Vector3d temp(utm.easting, utm.northing, utm.altitude);
      zero_utm = temp;

      Eigen::AngleAxisd v(M_PI * (zero_yaw - 90) / 180, Eigen::Vector3d::UnitZ());
      rotMatrix = v.matrix();
...
private:
  ...
  void initialpose_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& pose_msg) {
    ...
    ROS_INFO("I have received: [%f] [%f] [%f][%f]", q.w, q.x, q.y, q.z);
  ...
  void heading_callback(const geometry_msgs::QuaternionStamped& heading_msg) {
    double yaw = tf::getYaw(heading_msg.quaternion);
    //ROS_INFO(" yaw: [%f]", yaw);

    yaw =  - (yaw - (M_PI * zero_yaw/ 180));
  ...
  void navsat_callback(const sensor_msgs::NavSatFixConstPtr& navsat_msg) {
    ...
    if (!gnss_init || (navsat_msg->status.status == -1) || gnss_location) {
      // std::cout << "navsat_callback (gnss_init = " << gnss_init << "), navsat_msg->status.status = (" << navsat_msg->status.status << ") and  (gnss_location = " << gnss_location
      //        << ")" << std::endl;
      return;
    }

    if (!globalmap || !pose_estimator) {
      std::cout << "globalmap has not been received or pose has not been init" << std::endl;
    ...
    pose_estimator.reset(
      new hdl_localization::PoseEstimator(registration, ros::Time::now(), Eigen::Vector3f(xyz[0], xyz[1], 0), q_heading, private_nh.param<double>("cool_time_duration", 0.5)));
    gnss_location = true;
